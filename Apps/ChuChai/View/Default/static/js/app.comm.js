...
(function(fun)
{
      ...
      cfg =
  {
    ...
    paths:
    {
      ...
      'layer'                : cdn + 'layer/2.4/layer.min',
      'layer-css'            : cdn + 'layer/2.4/skin/layer.min',
    ...
    shim:
    {
      ...
      'layer'                : {deps:['css!layer-css'],exports:'layer'},
...
// JQ
  ...
  .on('click','.act-live-play',function()
  {
    ...
    ele.after('<div class="input-group"><span class="input-group-addon">拉流地址</span><input value="' + src + '" class="form-control"></div>');
    if(window.videojs)
    {
  ...
  (function()
  {
    ...
    $('.popover-tip').each(function()
    {
      var the = $(this),
          con = the.data('content') || the.attr('title');
      the.popover(
      {
        content:con,
        html:true,
    ...
    .on('mouseenter focus','.popover-with-ajax',function()
    {
      ...
      else require(['art-template','webui-popover'],function(art)
      {
        ...
        the.webuiPopover(
        {
          ...
          onShow:function(the)
          {
            the.find('.date-time,.date-range').trigger('init');
          }
        }).webuiPopover('show');
      });
    })
  ...
  (function(fun)
  {
    ...
    body.on('init','.date-time',function()
    {
      var the = $(this);
    ...
    if(rng.length) require(['moment'],function(moment)
    {
      ...
      rng.not('.date-time-ranges-noauto').each(function()
      {
        var the = $(this);
        the.append(rls.join(''));
      });
      ...
      {
        var the = $(this),
            box = the.closest('.input-group,form').has('.date-time,[name="stime"],[name="etime"]');
        box.find('[name="stime"],.date-time:eq(0)').val(the.data('stime') || '');
        box.find('[name="etime"],.date-time:eq(1)').val(the.data('etime') || '');
        e.preventDefault();
      });
    });
  })
  ...
  (function(fun)
  {
    ...
    body.on('init','.date-range',function()
    {
      var the = $(this);
